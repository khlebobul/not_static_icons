...
/// Animated Clipboard Minus Icon - Minus line pulses
class ClipboardMinusIcon extends AnimatedSVGIcon {
  const ClipboardMinusIcon({
    super.key,
    super.size = 40.0,
    super.color,
    super.hoverColor,
...
class ClipboardMinusPainter extends CustomPainter {
  final Color color;
  final double animationValue;
  final double strokeWidth;

  ClipboardMinusPainter({
    required this.color,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final lineScale = 1.0 + oscillation * 0.15;

    canvas.save();
    canvas.translate(12 * scale, 14 * scale);
    canvas.scale(lineScale, 1.0);
    canvas.translate(-12 * scale, -14 * scale);

    canvas.drawLine(
      Offset(9 * scale, 14 * scale),
      Offset(15 * scale, 14 * scale),
      paint,
    );
  ...
  @override
  bool shouldRepaint(ClipboardMinusPainter oldDelegate) {
