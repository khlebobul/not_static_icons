...
/// Animated File Type Icon - type cursor grows
class FileTypeIcon extends AnimatedSVGIcon {
  const FileTypeIcon({
    super.key,
    super.size = 40.0,
    super.color,
  ...
  @override
  String get animationDescription => "type cursor grows";
...
class FileTypePainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.save();
    final center = Offset(12 * scale, 19 * scale);
    canvas.translate(center.dx, center.dy);
    canvas.scale(1, 1 + pulse * .12);
    canvas.translate(-center.dx, -center.dy);
    final path2 = Path();
    path2.moveTo(11 * scale, 18 * scale);
    path2.lineTo(13 * scale, 18 * scale);
    canvas.drawPath(path2, paint);
    final path3 = Path();
    path3.moveTo(12 * scale, 12 * scale);
    path3.lineTo(12 * scale, 18 * scale);
    canvas.drawPath(path3, paint);
    final path4 = Path();
    path4.moveTo(9 * scale, 13 * scale);
    path4.lineTo(9 * scale, 12.5 * scale);
    path4.arcToPoint(Offset(9.5 * scale, 12 * scale),
        radius: Radius.circular(0.5 * scale));
    path4.lineTo(14.5 * scale, 12 * scale);
    path4.arcToPoint(Offset(15 * scale, 12.5 * scale),
        radius: Radius.circular(0.5 * scale));
    path4.lineTo(15 * scale, 13 * scale);
    canvas.drawPath(path4, paint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(FileTypePainter oldDelegate) {
