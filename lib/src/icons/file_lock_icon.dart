...
/// Animated File Lock Icon - lock shakes
class FileLockIcon extends AnimatedSVGIcon {
  const FileLockIcon({
    super.key,
    super.size = 40.0,
    super.color,
  ...
  @override
  String get animationDescription => "lock shakes";
...
class FileLockPainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path0 = Path();
    path0.moveTo(4 * scale, 9.8 * scale);
    ...
    path0.arcToPoint(Offset(18 * scale, 22 * scale),
        radius: Radius.circular(2 * scale));
    path0.lineTo(15 * scale, 22 * scale);
    ...
    canvas.save();
    final hinge = Offset(5 * scale, 17 * scale);
    canvas.translate(hinge.dx, hinge.dy);
    canvas.rotate(-pulse * .2);
    canvas.translate(-hinge.dx, -hinge.dy);
    final path2 = Path();
    path2.moveTo(9 * scale, 17 * scale);
    path2.lineTo(9 * scale, 15 * scale);
    path2.arcToPoint(Offset(5 * scale, 15 * scale),
        radius: Radius.circular(2 * scale), clockwise: false);
    path2.lineTo(5 * scale, 17 * scale);
    canvas.drawPath(path2, paint);
    canvas.restore();
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromLTWH(3 * scale, 17 * scale, 8 * scale, 5 * scale),
            Radius.elliptical(1 * scale, 1 * scale)),
        paint);
  }

  @override
  bool shouldRepaint(FileLockPainter oldDelegate) {
