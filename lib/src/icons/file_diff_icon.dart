...
/// Animated File Diff Icon - diff signs separate
class FileDiffIcon extends AnimatedSVGIcon {
  const FileDiffIcon({
    super.key,
    super.size = 40.0,
    super.color,
  ...
  @override
  String get animationDescription => "diff signs separate";
...
class FileDiffPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path1 = Path();
    path1.moveTo(9 * scale, 10 * scale);
    path1.lineTo(15 * scale, 10 * scale);
    canvas.drawPath(path1, paint);
    final path2 = Path();
    path2.moveTo(12 * scale, 13 * scale);
    path2.lineTo(12 * scale, 7 * scale);
    canvas.drawPath(path2, paint);
    canvas.restore();
    canvas.save();
    canvas.translate(0, pulse * 1.2 * scale);
    final path3 = Path();
    path3.moveTo(9 * scale, 17 * scale);
    path3.lineTo(15 * scale, 17 * scale);
