...
/// Animated File Code Corner Icon - corner code slides
class FileCodeCornerIcon extends AnimatedSVGIcon {
  const FileCodeCornerIcon({
    super.key,
    super.size = 40.0,
    super.color,
    super.hoverColor,
  ...
  @override
  String get animationDescription => "corner code slides";
...
class FileCodeCornerPainter extends CustomPainter {
  final Color color;
  final double animationValue;
  final double strokeWidth;

  FileCodeCornerPainter({
    required this.color,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path0 = Path();
    path0.moveTo(4 * scale, 12.15 * scale);
    ...
    path0.arcToPoint(Offset(18 * scale, 22 * scale),
        radius: Radius.circular(2 * scale));
    path0.lineTo(14.65 * scale, 22 * scale);
    ...
    canvas.save();
    canvas.translate(pulse * .7 * scale, 0);
    final path2 = Path();
    path2.moveTo(5 * scale, 16 * scale);
    path2.lineTo(2 * scale, 19 * scale);
    path2.lineTo(5 * scale, 22 * scale);
    canvas.drawPath(path2, paint);
    final path3 = Path();
    path3.moveTo(9 * scale, 22 * scale);
    path3.lineTo(12 * scale, 19 * scale);
    path3.lineTo(9 * scale, 16 * scale);
    canvas.drawPath(path3, paint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(FileCodeCornerPainter oldDelegate) {
