...
/// Animated File Symlink Icon - symlink jumps
class FileSymlinkIcon extends AnimatedSVGIcon {
  ...
  @override
  String get animationDescription => "symlink jumps";
...
class FileSymlinkPainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path0.arcToPoint(Offset(4 * scale, 20 * scale),
        radius: Radius.circular(2 * scale));
    path0.lineTo(4 * scale, 17 * scale);
    path0.arcToPoint(Offset(6 * scale, 15 * scale),
        radius: Radius.circular(2 * scale));
    path0.lineTo(13 * scale, 15 * scale);
    ...
    canvas.save();
    canvas.translate(pulse * 1.0 * scale, -pulse * .6 * scale);
    final path2 = Path();
    path2.moveTo(10 * scale, 18 * scale);
    path2.lineTo(13 * scale, 15 * scale);
    path2.lineTo(10 * scale, 12 * scale);
    canvas.drawPath(path2, paint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(FileSymlinkPainter oldDelegate) {
