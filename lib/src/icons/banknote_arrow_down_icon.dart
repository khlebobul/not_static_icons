...
/// Animated Banknote Arrow Down Icon - Starts fully visible, then disappears and redraws with animation
class BanknoteArrowDownIcon extends AnimatedSVGIcon {
  ...
  String get animationDescription =>
      "Banknote with arrow down: starts visible, disappears, then redraws banknote and arrow in sequence";
  ...
  CustomPainter createPainter({
    ...
    return BanknoteArrowDownPainter(
      color: color,
...
/// Painter for Banknote Arrow Down icon
class BanknoteArrowDownPainter extends CustomPainter {
  final Color color;
  final double animationValue;
  final double strokeWidth;

  BanknoteArrowDownPainter({
    required this.color,
    ...
    if (animationValue == 0.0) {
      ...
      if (redrawValue >= 0.77) {
        if (redrawValue >= 0.88) {
          ...
          canvas.drawLine(Offset(19 * scale, 16 * scale),
              Offset(19 * scale, 22 * scale), paint);
        } else {
      ...
      if (redrawValue >= 0.88) {
        if (redrawValue >= 0.9) {
          ...
          canvas.drawLine(Offset(16 * scale, 19 * scale),
              Offset(19 * scale, 22 * scale), paint);
          canvas.drawLine(Offset(19 * scale, 22 * scale),
              Offset(22 * scale, 19 * scale), paint);
        } else {
  ...
  void _drawCompleteIcon(Canvas canvas, double scale, Paint paint) {
    ...
    // Arrow shaft (M19 16v6)
    canvas.drawLine(
        Offset(19 * scale, 16 * scale), Offset(19 * scale, 22 * scale), paint);

    // Arrowhead (m16 19 3 3 3-3)
    canvas.drawLine(
        Offset(16 * scale, 19 * scale), Offset(19 * scale, 22 * scale), paint);
    canvas.drawLine(
        Offset(19 * scale, 22 * scale), Offset(22 * scale, 19 * scale), paint);
  }
  ...
  void _drawArrowShaftAnimated(
      Canvas canvas, double scale, Paint paint, double progress) {
    final start = Offset(19 * scale, 16 * scale);
    final end = Offset(19 * scale, 22 * scale);
  ...
  void _drawArrowHeadAnimated(
    ...
    final tip = Offset(19 * scale, 22 * scale);
  ...
  @override
  bool shouldRepaint(BanknoteArrowDownPainter oldDelegate) {
