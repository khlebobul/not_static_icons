...
/// Animated File Check Corner Icon - corner check snaps in
class FileCheckCornerIcon extends AnimatedSVGIcon {
  ...
  @override
  String get animationDescription => "corner check snaps in";
...
class FileCheckCornerPainter extends CustomPainter {
  final Color color;
  final double animationValue;
  final double strokeWidth;

  FileCheckCornerPainter({
    required this.color,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path0 = Path();
    path0.moveTo(10.5 * scale, 22 * scale);
    ...
    path0.arcToPoint(Offset(20 * scale, 8 * scale),
        radius: Radius.circular(2.4 * scale));
    path0.lineTo(20 * scale, 14 * scale);
    ...
    canvas.save();
    final center = Offset(17 * scale, 20 * scale);
    canvas.translate(center.dx, center.dy);
    canvas.scale(1 + pulse * .16);
    canvas.translate(-center.dx, -center.dy);
    final path2 = Path();
    path2.moveTo(14 * scale, 20 * scale);
    path2.lineTo(16 * scale, 22 * scale);
    path2.lineTo(20 * scale, 18 * scale);
    canvas.drawPath(path2, paint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(FileCheckCornerPainter oldDelegate) {
