...
class CalculatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(8 * scale, 6 * scale), Offset(16 * scale, 6 * scale), paint);

    // Vertical Line
    // line x1="16" x2="16" y1="14" y2="18"
    canvas.drawLine(Offset(16 * scale, 14 * scale), Offset(16 * scale, 18 * scale), paint);
    ...
    for (int i = 0; i < buttons.length; i++) {
      ...
      if (animationValue > 0 && animationValue >= start && animationValue < end) {
        isPressed = true;
      }
