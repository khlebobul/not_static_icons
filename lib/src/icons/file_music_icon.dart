...
/// Animated File Music Icon - music note hops
class FileMusicIcon extends AnimatedSVGIcon {
  const FileMusicIcon({
    super.key,
    super.size = 40.0,
    super.color,
  ...
  @override
  String get animationDescription => "music note hops";
...
class FileMusicPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path0 = Path();
    path0.moveTo(11.65 * scale, 22 * scale);
    ...
    path0.arcToPoint(Offset(4 * scale, 4 * scale),
        radius: Radius.circular(2 * scale), clockwise: false);
    path0.lineTo(4 * scale, 14.35 * scale);
    ...
    canvas.save();
    canvas.translate(0, -pulse * 1.2 * scale);
    final path2 = Path();
    path2.moveTo(8 * scale, 20 * scale);
    path2.lineTo(8 * scale, 13 * scale);
    path2.lineTo(11 * scale, 14.474 * scale);
    canvas.drawPath(path2, paint);
    canvas.drawCircle(Offset(6 * scale, 20 * scale), 2 * scale, paint);
    canvas.restore();
  }
