...
/// Animated File Terminal Icon - terminal prompt blinks forward
class FileTerminalIcon extends AnimatedSVGIcon {
  ...
  @override
  String get animationDescription => "terminal prompt blinks forward";
...
class FileTerminalPainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.save();
    canvas.translate(pulse * .8 * scale, 0);
    final path2 = Path();
    path2.moveTo(8 * scale, 16 * scale);
    path2.lineTo(10 * scale, 14 * scale);
    path2.lineTo(8 * scale, 12 * scale);
    canvas.drawPath(path2, paint);
    final path3 = Path();
    path3.moveTo(12 * scale, 18 * scale);
    path3.lineTo(16 * scale, 18 * scale);
    canvas.drawPath(path3, paint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(FileTerminalPainter oldDelegate) {
