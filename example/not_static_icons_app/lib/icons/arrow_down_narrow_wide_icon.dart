...
class ArrowDownNarrowWideIcon extends AnimatedSVGIcon {
  const ArrowDownNarrowWideIcon({
    super.key,
    super.size = 40,
  ...
  CustomPainter createPainter({
    ...
    return _ArrowDownNarrowWidePainter(
      color: color,
  ...
  String get animationDescription =>
      'Lines disappearing and reappearing from narrow to wide';
}

class _ArrowDownNarrowWidePainter extends CustomPainter {
  final Color color;
  final double animationValue;
  final double strokeWidth;

  _ArrowDownNarrowWidePainter({
    required this.color,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (animationValue == 0) {
      ...
      // Lines appear in order: shortest -> medium -> longest
      double progress = (animationValue - 0.2) / 0.6;
      opacity1 = (progress * 3).clamp(0.0, 1.0); // Shortest appears first
    ...
    line3Path.lineTo(21, 12);
    canvas.drawPath(line3Path, paint3);
  }

  @override
  bool shouldRepaint(covariant _ArrowDownNarrowWidePainter oldDelegate) {
